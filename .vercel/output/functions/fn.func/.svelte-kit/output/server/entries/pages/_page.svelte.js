import { c as create_ssr_component, d as compute_rest_props, a as setContext, f as spread, h as escape_object, i as escape_attribute_value, j as add_attribute, g as getContext, e as escape, v as validate_component, k as compute_slots, l as createEventDispatcher } from "../../chunks/ssr.js";
...
const Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  return `<header><nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800"><div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl"><div class="flex items-center">${validate_component(Avatar, "Avatar").$$render(
    ...
    {},
    {}
  )} <span class="self-center text-4xl font-semibold whitespace-nowrap text-gray-700 dark:text-white" data-svelte-h="svelte-12u6gnn">Sollicitatie PentAgora Docent</span></div> <div class="flex items-center lg:order-2">${validate_component(DarkMode, "DarkMode").$$render($$result, { btnClass: darkmodebtn }, {}, {})}</div></div></nav></header>`;
});
...
let TitleComp = "Mens";
let QuoteComp = "Je bent niet voor jezelf op de wereld, maar voor de ander";
let TextComp = "<p>Graag wil ik mij voorstellen: ik ben Jeroen geboren in Vlaardingen en opgegroeid in Spijkenisse. Ik ben echtgenote van Anja en vader van een tweeling Jelke en Meie en een zoon Noek. Ik ben van jongs af aan al veel met sport en bewegen bezig. Op dit moment doe ik zelf graag hardlopen, doe ik aan golfsurfen, snowboarden, wielrennen en skaten.</p><p>Mijn interesse ligt verder bij maatschappelijk kwesties, hou ik van technologische ontwikkelingen, video editing, maken van websites. Ik ben iemand die van een uitdaging houdt en open staat om nieuwe dingen te leren. </p><p>Ik ben vooral die aardige gast die openstaat, makkelijk connectie maakt, interesse toont, mensen probeert te inspireren en te verbinden.</p>";
let BtnColorComp = "cyan";
const Comp = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  let ModalName = false;
  let { ImgComp } = $$props;
  if ($$props.ImgComp === void 0 && $$bindings.ImgComp && ImgComp !== void 0)
    $$bindings.ImgComp(ImgComp);
  let $$settled;
  let $$rendered;
  ...
  do {
    ...
    $$rendered = `<div>${validate_component(Card, "Card").$$render(
      ...
      {
        default: () => {
          return `<h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-600 dark:text-gray-400">${escape(TitleComp)}</h5> <p class="mb-3 font-normal italic text-gray-600 dark:text-gray-400 leading-tight">“${escape(QuoteComp)}”</p> ${validate_component(GradientButton, "GradientButton").$$render($$result, { color: BtnColorComp, autoclose: true }, {}, {
            default: () => {
              return `Lees Meer ${validate_component(ArrowRightOutline, "ArrowRightOutline").$$render($$result, { class: "w-3.5 h-3.5 ml-2 text-white" }, {}, {})}`;
            }
          })}`;
        }
      }
